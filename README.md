...
not available. `npm run build`, `npm run desktop`, tests, and development mode remain usable
...
and `.zip` plus `latest-mac.yml` in the same run — no second dispatch needed. The draft
remains the human publish checkpoint: verify the artifacts and notes, then publish it.

The macOS release job requires these GitHub Actions secrets (the certificate and API key are
stored as base64-encoded values):

- `MACOS_CERTIFICATE_P12` and `MACOS_CERTIFICATE_PASSWORD` for the Developer ID
  Application certificate.
- `APPLE_API_KEY_P8`, `APPLE_API_KEY_ID`, and `APPLE_API_ISSUER` for App Store Connect
  API-key notarization.

The first signed release is a manual bootstrap. Configure those secrets, run the release
workflow, and publish the resulting draft only after Gatekeeper and notarization checks pass.
That release supplies the updater metadata and signed ZIP needed by later in-app updates.
...
automatically. Publishing is the one thing left to a human: it's the checkpoint to confirm
the artifacts attached and the notes read right before going live.
...
an "Update to vX.Y.Z" badge linking to the release page. The result is cached for six hours.
The running version remains available from the top-bar badge and `GET /api/version`.

The updater reads the GitHub provider metadata generated by electron-builder and uploaded as
`latest-mac.yml` alongside each release's DMG and ZIP. It is intentionally limited to the
signed macOS arm64 distribution; other environments keep the release-page notification.
